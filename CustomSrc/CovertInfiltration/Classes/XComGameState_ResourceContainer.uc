...
function string GetCommaSeparatedContents ()
{
	...
	if(Packages.Length == 0)
		`log("XComGameState_ResourceContainer::GetCommaSeparatedContents: No packages to read from!");
	...
	foreach Packages(Package)
	{
		...
		{
			`log("Reading package:" @ Package.ItemType);
			if(Result == "")
			{
	...
	if(Result == "")
		`log("XComGameState_ResourceContainer::GetCommaSeparatedContents: Something went wrong in reward string!");
