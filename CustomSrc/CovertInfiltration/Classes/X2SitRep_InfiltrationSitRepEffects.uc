...
)
{
	...
	{
		`log("EmplaceFollowerIntoEncounter got EncounterName that doesn't exist - aborting");
		return;
	}
...
static function MessyInsertionTacticalStartModifier(XComGameState StartState)
{
	...
	for (LoopIdx = 0; LoopIdx < UnitRefs.Length; LoopIdx++)
	{
		...
		if (UnitState != none)
		{
			...
			{
				`log("Super Unlucky Soldier: " $ LoopIdx);

				`log("Health: " $ UnitState.GetCurrentStat(eStat_HP));
