...
static function name GetInfilName(name ElementName, EInfilModifierType ElementType)
{
	...
	switch (ElementType)
	{
		...
		case eIMT_Character:
			...
			break;
		default:
			`log("X2InfiltrationMod failed to grab type for " $ string(ElementName));
			return '';
	}
...
static function array<X2DataTemplate> CreateTemplates()
{
	...
	foreach default.InfilModifiers(Modifier)
	{
		...
		{
			`log("X2InfiltrationModTemplate" @ string(TemplateName) @ "requires" @ Modifier.DLC @ "which is not loaded - skipping creation");
			continue;
		}
