...
function SetupChain (XComGameState NewGameState)
{
	...
	`log("Setting up chain" @ m_TemplateName);
	...
	`log("Selected faction and regions, spawning stages");
	...
	foreach m_Template.Stages(StageDef, i)
	{
		...
		{
			`log("Stage definition is invalid! Cannot spawn activity: " $ ActivityTemplateName);
		}
	}

	`log("Spawned stages, calling lifecycle callbacks");
...
function StartNextStage (XComGameState NewGameState)
{
	...
	`log("Starting stage" @ iCurrentStage @ "of" @ m_TemplateName);
...
function CurrentStageHasCompleted (XComGameState NewGameState)
{
	...
	`log(m_TemplateName @ "current stage has reported completion, processing chain reaction");

	// Check if can progress
	if (iCurrentStage < StageRefs.Length - 1)
	{
		`log("Still more stages avaliable");

		ActivityState = GetCurrentActivity();

		if (ActivityState.ShouldProgressChain())
		{
			`log("Progression not blocked by stage template");
			StartNextStage(NewGameState);
		}
		else
		{
			`log("Progression blocked by stage template");
	...
	if (bEnded)
	{
		`log("Chain ended, calling lifecycle callbacks");
	...
	`log("Finished handling stage completion");
}
	...
	if (m_Template.bAllowComplications)
	{
		...
		foreach TemplateManager.IterateTemplates(DataTemplate)
		{
			...
			if (CanComplicationBeSelected(NewGameState, ComplicationTemplate))
			{
				...
				if (AttachmentRoll > 0)
				{
					`log("Adding Complication" @ ComplicationTemplate.DataName @ "at" @ AttachmentRoll $ "%");

					ActivationRoll = `SYNC_RAND_STATIC(100);
				
					`log("Complication Activation Roll: " $ ActivationRoll $ " < " $ AttachmentRoll);
				...
				{
					`log("Adding Complication" @ ComplicationTemplate.DataName @ "but roll failed");
				}
			}
		}
	}
...
protected function bool CanComplicationBeSelected (XComGameState NewGameState, X2ComplicationTemplate ComplicationTemplate)
{
	...
	{
		`log(ComplicationTemplate.FriendlyName $ " cannot be chosen");
		return false;
	}
	
	`log(ComplicationTemplate.FriendlyName $ " is chosen");
	return true;
}
...
function XComGameState_Reward ClaimChainReward (XComGameState NewGameState)
{
	...
	foreach UnclaimedChainRewardRefs(RewardRef)
	{
		...
		if (RewardState != none)
		{
			`log("Claiming chain reward: " $ RewardState.GetMyTemplateName());
...
function XComGameState_Activity GetActivityAtIndex (int i)
{
	...
	{
		`log("GetActivityAtIndex called with invalid index");
		ScriptTrace();
		return none;
	}
...
static protected function name PickActivityForStage (ChainStage StageDef)
{
	...
	{
		`log("Stage is preset -" @ StageDef.PresetActivity);
	...
	foreach TemplateManager.IterateTemplates(DataTemplate)
	{
		...
		`log("Activity Cadidate:" @ ActivityTemplate.DataName);
		...
		{
			`log("Weight mod:" @ StageDef.ActivityWeightMods[iMod].ModType @ StageDef.ActivityWeightMods[iMod].Value);
			...
			`log("Weight after mod:" @ Weight);
		}

		if (Weight <= 0)
		{
			`log("Weight <= 0, skpping");
			continue;
		}
		...
		`log("Candidate complete - " $ ActivityTemplate.DataName $ ". Weight = " $ Weight $ ". TotalWeight = " $ TotalWeight);
	}
	...
	foreach Candidates(Candidate)
	{
		...
		if (Roll < TotalWeight)
		{
			`log("Candidate selected - " $ Candidate.ActivityName $ ". Current TotalWeight = " $ TotalWeight $ ". Roll = " $ Roll);
			return Candidate.ActivityName;
		}
	}

	`RedScreen(nameof(PickActivityForStage) @ "internal error during weighted roll");
	`log("Code should not reach here. TotalWeight = " $ TotalWeight $ ". Roll = " $ Roll);
}
...
protected function DoRemove ()
{
	...
	`log("Starting removal of" @ m_TemplateName);
	...
	`log("Removal of" @ m_TemplateName @ "- processing activities");
	foreach StageRefs(ActivityRef)
	{
		ActivityState = XComGameState_Activity(NewGameState.ModifyStateObject(class'XComGameState_Activity', ActivityRef.ObjectID));
	...
	`log("Removal of" @ m_TemplateName @ "- processing complications");
	...
	`log("Completed removal of" @ m_TemplateName);
}
