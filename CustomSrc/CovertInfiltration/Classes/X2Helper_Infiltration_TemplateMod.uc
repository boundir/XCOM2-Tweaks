...
static function PatchInfiltrationTemplates()
{
	...
	foreach default.EditInfilModifiers(Modifier)
	{
		...
		{
			`log("X2InfiltrationModTemplate" @ string(Modifier.DataName) @ "requires" @ Modifier.DLC @ "which is not loaded - skipping edit");
			continue;
		}
		...
		if (bTemplateChanged)
		{
			`log("X2InfiltrationModTemplate" @ string(Modifier.DataName) @ "was successfully edited using EditInfilModifiers");
		}
		else
		{
			`log("X2InfiltrationModTemplate" @ string(Modifier.DataName) @ "could not be found for editing by EditInfilModifiers");
		}
	}
}
...
static protected function array<XComGameState_WorldRegion> GetRetaliationRegion (XComGameState NewGameState)
{
	...
	`log("GetRetaliationRegions Adjustment" @ Adjustment);
	...
	for (i = 0; i < RegionScores.Length; i++)
	{
		...
		`log("Region:" @ RegionScores[i].RegionState.GetMyTemplateName() $ "; Score:" @ RegionScores[i].Score $ "; TotalScore:" @ TotalScore);
	}

	Roll = `SYNC_RAND_STATIC(TotalScore + 1);
	`log("GetRetaliationRegions Roll" @ Roll);
...
static function RemoveSabotages ()
{
	...
	Manager = class'X2StrategyElementTemplateManager'.static.GetStrategyElementTemplateManager();
	`log("Disabling Chosen sabotages");
...
static function RemovePointsOfInterest ()
{
	...
	Manager = class'X2StrategyElementTemplateManager'.static.GetStrategyElementTemplateManager();
	`log("Disabling Points of Interest");
	...
	foreach default.arrPointsOfInterestToRemove(TemplateName)
	{
		...
		{
			`log(TemplateName $ " is not a POI template, cannot disable!");
		}
	}
}
