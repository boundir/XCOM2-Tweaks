...
{
	`log(nameof(SetAnchor) @ "is invalid on" @ class.Name);
	return self;
}

simulated function UIPanel SetSize (float NewWidth, float NewHeight)
{
	`log(nameof(SetSize) @ "is invalid on" @ class.Name);
	return self;
}

simulated function UIPanel SetPosition (float NewX, float NewY)
{
	`log(nameof(SetPosition) @ "is invalid on" @ class.Name);
	return self;
}

simulated function UIPanel SetPanelScale (float NewScale)
{
	`log(nameof(SetPanelScale) @ "is invalid on" @ class.Name);
	return self;
}
