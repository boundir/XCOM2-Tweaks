...
function InitializeFromActivity (XComGameState NewGameState)
{
	...
	`log("InitializeFromActivity reports " $ Rewards.Length $ " rewards");
...
function UpdateGameBoard()
{
	...
	if (BonusTemplate != none && GetCurrentInfilInt() >= GetNextThreshold())
	{
		...
		`log("Checking if should pause at: " $ BonusTemplate.Milestone);
...
function int GetChosenAppereanceChance()
{
	...
	{
		`log("GetChosenAppereanceChance - returning -1 as there is no chosen to spawn or chosen are not activated");
		return -1;
	}
	...
	if (IsRulerPresent())
	{
		`log("GetChosenAppereanceChance - returning 0 as there is a ruler on this mission");
		return 0;
	}
	...
	`log("ChosenAppereanceChance: BaseChance=" $ BaseChance $ ", OverInfilScalar=" $ OverInfilScalar $ ", AlienHQScalar=" $ AlienHQScalar);
...
function bool ShouldPreformChosenRoll ()
{
	...
	{
		`log("Chosen roll not performed yet or reset after another mission was done");
		return true;
	}

	if (Abs(ChosenRollLastDoneAt - GetCurrentInfilInt()) >= ChosenRollInfilInterval)
	{
		`log("Infiltration progressed further than chosen roll interval");
		return true;
	}

	if (CurrentChosen != none && CurrentChosen.Level != ChosenLevel)
	{
		`log("Chosen level has changed");
		return true;
	}

	`log("No reason found to reroll the chosen");
	return false;
}
...
function PreformChosenRoll ()
{
	...
	{
		`log("No chosen active for this infiltration");
	...
	else
	{
		...
		`log("Chosen roll chance" @ AppearenceChance @ (bNewShouldSpawn ? "- success" : "- failed"));
	}
...
function CacheDifficulty()
{
	...
	{
		`log("No difficulty function to cache Dfifficulty for Infiltration");
	}
}
