...
static protected function ModVersion_OnLoadedSavedGameWithDLCExisting ()
{
	...
	`log("ModVersion_OnLoadedSavedGameWithDLCExisting running");
...
static protected function ModVersion_FinalizeStrategy ()
{
	...
	`log("ModVersion_FinalizeStrategy running");
...
static protected function FixInfilsWithoutSoldiers (XComGameState NewGameState)
{
	...
	foreach History.IterateByClassType(class'XComGameState_MissionSiteInfiltration', InfilState)
	{
		...
		{
			`log(GetFuncName() @ "infil" @ InfilState.ObjectID @ "skipped due to bRemoved");
			continue;
		}
		...
		{
			`log(GetFuncName() @ "infil" @ InfilState.ObjectID @ "skipped as it was the latest mission");
			continue;
		}
		...
		{
			`log(GetFuncName() @ "infil" @ InfilState.ObjectID @ "UnRegisterFromActionEvents");
		...
		{
			`log(GetFuncName() @ "infil" @ InfilState.ObjectID @ "skipped as it has not transitioned to mission stage yet");
			continue;
		}
		...
		{
			`log(GetFuncName() @ "infil" @ InfilState.ObjectID @ "skipped as it has not lost its soldiers");
			continue;
		}

		// We found a broke
		`log(GetFuncName() @ "infil" @ InfilState.ObjectID @ "detected broken, patching");
		...
		{
			`log(GetFuncName() @ "infil" @ InfilState.ObjectID @ "found older version that soldiers set, attempting restore");
			...
			foreach OlderInfilState.SoldiersOnMission(UnitRef)
			{
				...
				`log(GetFuncName() @ "infil" @ InfilState.ObjectID @ "added unit" @ UnitRef.ObjectID);
			}
	...
	foreach `XCOMHQ.Crew(UnitRef)
	{
		...
		`log(GetFuncName() @ "unit" @ UnitRef.ObjectID @ "is in infil limbo, unstaffing");
...
private static function bool IsUnitInInfilLimbo (StateObjectReference UnitRef, XComGameState CheckGameState)
{
	...
	{
		`log(GetFuncName() @ "unit" @ UnitRef.ObjectID @ "no longer exists (???)");
		return false;
	}
	...
	{
		`log(GetFuncName() @ "unit" @ UnitRef.ObjectID @ "is not staffed into an InfiltrationStaffSlot");
		return false;
	}
	...
	foreach `XCOMHISTORY.IterateByClassType(class'XComGameState_MissionSiteInfiltration', OtherInfilState)
	{
		...
		{
			`log(GetFuncName() @ "unit" @ UnitRef.ObjectID @ "is on an infil" @ OtherInfilState.ObjectID);
			return false;
		}
	}
...
static protected function PatchSupplyExactionCrates ()
{
	...
	foreach default.SupplyExtractionLootTables(LootTable)
	{
		...
		for (iLootEntry = 0; iLootEntry < LootManager.LootTables[iTable].Loots.Length; iLootEntry++)
		{
			...
			{
				`log(GetFuncName() @ `showvar(LootTable) @ `showvar(iLootEntry) @ `showvar(EntryTemplateName) @ "is not permitted to be modified");
				continue;
			}

			`log(GetFuncName() @ `showvar(LootTable) @ `showvar(EntryTemplateName) @ "pre  patch - Min" @ LootManager.LootTables[iTable].Loots[iLootEntry].MinCount @ "Max" @ LootManager.LootTables[iTable].Loots[iLootEntry].MaxCount);
			...
			`log(GetFuncName() @ `showvar(LootTable) @ `showvar(EntryTemplateName) @ "post patch - Min" @ LootManager.LootTables[iTable].Loots[iLootEntry].MinCount @ "Max" @ LootManager.LootTables[iTable].Loots[iLootEntry].MaxCount);
		}
	}
}
...
static protected function TryEnlistChainStateIntoTactical (XComGameState StartGameState, XComGameState_MissionSite MissionState)
{
	...
	{
		`log("Mission not associated with an activity, skipping enlisting states into tactical");
		return;
	}

	`log("Enlisting chain (and related states) into tactical start state");
...
static protected function OnPreMission_Activity (XComGameState StartGameState, XComGameState_MissionSite MissionState)
{
	...
	{
		`log("ERROR: Launching into an activity-associated mission, but the activity doesn't use X2ActivityTemplate_Mission???");
		return;
	}
...
static protected function ResetAssaultChosenRoll ()
{
	...
	foreach History.IterateByClassType(class'XComGameState_MissionSite', MissionSite)
	{
		...
		{
			`log(GetFuncName() @ MissionSite.ObjectID @ "latest mission");
			continue;
		}
		...
		{
			`log(GetFuncName() @ MissionSite.ObjectID @ "infil");
			continue;
		}
		...
		{
			`log(GetFuncName() @ MissionSite.ObjectID @ "ShouldManageChosenOnAssault is false");
			continue;
		}
...
static protected function PostChosenStronghold ()
{
	...
	`log("DisableChosenSurveillance called");
...
static protected function OnExitPostMissionSequence_Activity ()
{
	...
	if (MissionActivityTemplate == none)
	{
		`log("ERROR: OnExitPostMissionSequence_Activity: the activity doesn't use X2ActivityTemplate_Mission???");
		return;
	}
...
static function PostEncounterCreation (out name EncounterName, out PodSpawnInfo Encounter, int ForceLevel, int AlertLevel, optional XComGameState_BaseObject SourceObject)
{
	...
	if (MissionState == none)
	{
		`log("PostEncounterCreation recived SourceObject which is not instance of XComGameState_MissionSite");
		return;
	}
...
exec function SpawnActivityChain (name ChainTemplateName, int StartAtStage = 0)
{
	...
	`log("Starting cheat: SpawnActivityChain" @ ChainTemplateName);
...
exec function SetNumKillsOfCharacterGroup (name CharacterGroup, int NewKills)
{
	...
	`log("Set kill count for" @ CharacterGroup @ "to" @ CIInfo.GetCharacterGroupsKills(CharacterGroup));
}
...
exec function SpawnResistanceCardAction (name CardName)
{
	...
	{
		`log("SpawnResistanceCardAction: Failed to find" @ CardName @ "card");
		return;
	}

	if (DesiredCardState.bDrawn)
	{
		`log("SpawnResistanceCardAction: Card" @ CardName @ "is already drawn, cannot spawn CA for it");
		return;
	}
	...
	if (FactionState == none)
	{
		`log("SpawnResistanceCardAction: Card" @ CardName @ "does not belong to a faction, cannot spawn CA for it");
		return;		
	}
	...
	if (ActionTemplate == none)
	{
		`log("SpawnResistanceCardAction: Failed to find CA template, cannot spawn CA");
		return;
	}
...
exec function SetBonusForInfiltrationMilestone (name Milestone, name Bonus)
{
	...
	{
		`log("SetBonusForInfiltrationMilestone failed - not looking at infiltration mission blades");
		return;
	}
	...
	if (i == INDEX_NONE)
	{
		`log("SetBonusForInfiltrationMilestone failed - invalid milestone");
		return;
	}

	if (InfiltrationState.SelectedInfiltartionBonuses[i].bGranted)
	{
		`log("SetBonusForInfiltrationMilestone failed - milestone already granted");
		return;
	}
	...
	{
		`log("SetBonusForInfiltrationMilestone failed - invalid bonus");
		return;
	}

	if (BonusTemplate.Milestone != Milestone)
	{
		`log("SetBonusForInfiltrationMilestone failed - bonus milestone mismatch");
		return;
	}
...
exec function SetFlatRisk (name FlatRiskName)
{
	...
	{
		`log("SetFlatRisk failed - invalid FlatRiskName");
		return;
	}
	...
	{
		`log("SetFlatRisk failed - risk template not found");
		return;
	}
	...
	{
		`log("SetFlatRisk failed - not looking at covert actions screen");
		return;
	}
	...
	if (ActionState.bStarted)
	{
		`log("SetFlatRisk failed - action already started");
		return;
	}

	if (!class'X2Helper_Infiltration'.static.IsInfiltrationAction(ActionState))
	{
		`log("SetFlatRisk failed - not looking at an infiltration action");
		return;
	}
...
exec function SetRiskOccurance (name RiskName, bool bOccurs)
{
	...
	if (ActionsScreen == none)
	{
		`log("SetRiskActivation failed - not looking at covert actions screen");
		return;
	}
	...
	{
		`log("SetRiskActivation failed - action is not ongoing");
		return;
	}

	if (ActionState.NegatedRisks.Find(RiskName) != INDEX_NONE)
	{
		`log("SetRiskActivation failed - risk is nagated");
		return;
	}
	...
	if (i == INDEX_NONE)
	{
		`log("SetRiskActivation failed - risk does not exist on the action");
		return;
	}
...
exec function ForceAbortSelectedInfil ()
{
	...
	if (MissionScreen == none)
	{
		`log("ForceAbortSelectedInfil failed - not looking at infiltration mission blades");
		return;
	}
...
exec function AttachComplicationToFocusedChain (name ComplicationName, optional bool bActivated = true, optional int TriggerChance = 0)
{
	...
	{
		`log("AttachComplicationToFocusedChain failed - not looking at chains overview screen");
		return;
	}
	...
	{
		`log("AttachComplicationToFocusedChain failed - no chain selected");
		return;
	}
	...
	if (ChainState == none) 
	{
		`log("AttachComplicationToFocusedChain failed - failed to fetch chain state");
		return;
	}

	if (!ChainState.GetMyTemplate().bAllowComplications)
	{
		`log("AttachComplicationToFocusedChain failed - chain forbids complications");
		return;
	}
	...
	{
		`log("AttachComplicationToFocusedChain failed - complication not found");
		return;
	}
...
{
	`log("DisableChosenSurveillance activated from console command");
...
exec function CI_DumpCurrentArmoryFullName ()
{
	...
	`log(nameof(CI_DumpCurrentArmoryFullName) @ Screen.GetUnit().GetFullName());
}
...
exec function CI_SetCurrentArmorySoldierWill (float NewStat)
{
	...
	if (UnitState == none)
	{
		`log(nameof(CI_DumpCurrentArmoryFullName) @ "failed to fetch unit state, aborting");
		return;
	}
...
exec function DumpInfilProgress ()
{
	...
	if (MissionScreen == none)
	{
		`log(GetFuncName() @ "failed - not looking at" @ class'UIMission_Infiltrated'.Name);
		return;
	}

	MissionState = MissionScreen.GetInfiltration();

	`log(GetFuncName() @ `showvar(MissionState.MaxAllowedInfil));
	`log(GetFuncName() @ `showvar(MissionState.GetMaxAllowedInfilDecimal()));
	`log(GetFuncName() @ `showvar(MissionState.GetMaxAllowedOverInfil()));

	`log(GetFuncName() @ `showvar(MissionState.SecondsForOnePercent));
	`log(GetFuncName() @ `showvar(MissionState.GetCurrentOverInfil()));
	`log(GetFuncName() @ `showvar(MissionState.GetCurrentInfil()));
	`log(GetFuncName() @ `showvar(MissionState.GetCurrentInfilInt()));
	`log(GetFuncName() @ `showvar(MissionState.MustLaunch()));
	`log(GetFuncName() @ `showvar(MissionState.GetCurrentOverInfilPercentToMax()));
}
...
exec function TestChosenInfilScalar (int InfilProgress)
{
	...
	`log(GetFuncName() @ `showvar(InfilProgress) @ `showvar(Scalar));
}
...
static function DisableChosenSurveillance ()
{
	...
	`log("DisableChosenSurveillance passed");
	...
	foreach History.IterateByClassType(class'XComGameState_ActivityChain', ChainState)
	{
		...
		`log("DisableChosenSurveillance on chain" @ ChainState.GetMyTemplateName());

		if (NewGameState == none) 
		{
			`log("DisableChosenSurveillance creating gamestate");
	...
	if (NewGameState != none)
	{
		`log("DisableChosenSurveillance submitting gamestate");
		`XCOMGAME.GameRuleset.SubmitGameState(NewGameState);
	}
}
