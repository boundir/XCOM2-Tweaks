...
static function float GetKillContributionMultiplerForKill (name VictimCharacterGroup)
{
	...
	`log("GroupKillsReferenceCount=" $ GroupKillsReferenceCount $ ", VictimCharacterGroup=" $ VictimCharacterGroup $ ", NumKills=" $ NumKills);
...
static function HandlePostMissionPOI(XComGameState NewGameState, XComGameState_Activity ActivityState, bool bSuccess)
{
	...
	if (Template != none && Template.bNeedsPOI)
	{
		if (bSuccess)
		{
			...
			{
				MissionState.PickPOI(NewGameState);
				`log(ActivityState.GetMyTemplateName() $ " has no POI and is marked as requiring one! Spawning replacement");
			}
...
static function bool UnitHasIrrelevantItems (StateObjectReference UnitRef)
{
	...
	`log("BEGINNING IRRELEVANT ITEMS CHECK");

	// loop through items
	CurrentInventory = UnitState.GetAllInventoryItems();
	foreach CurrentInventory(InventoryItem)
	{
		`log("  - ANALYZING ITEM: " $ InventoryItem.GetMyTemplateName());

		if (default.IRRELEVANT_ITEM_SLOTS.Find(InventoryItem.InventorySlot) == INDEX_NONE)
		{
			`log("  -   - ITEM IS NOT IN VALID SLOT");
			continue;
		}

		if (InventoryItem.GetMyTemplate().bInfiniteItem)
		{
			`log("  -   - ITEM IS INFINITE");
			continue;
		}
		...
		else
		{
			...
			if (InfilTemplate != none)
			{
				if (InfilTemplate.HoursAdded >= 0 && InfilTemplate.Deterrence <= 0)
				{
					`log("  -   - IRRELEVANT CATEGORY");
					return true;
				}
			}
			else
			{
				`log("  -   - IRRELEVANT UNKNOWN");
				return true;
			}
		}
	}
	
	`log("  - UNIT CLEARED OF IRRELEVANT ITEMS");
