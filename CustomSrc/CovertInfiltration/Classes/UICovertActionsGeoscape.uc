...
{
	`log("Opening UICovertActionsGeoscape");
...
simulated function FindActions()
{
	...
	`log("Found" @ arrActions.Length @ "actions");
}
...
simulated function UpdateCovertActionInfo()
{
	...
	{
		`log("Action is an infil, setting reward string");
		...
		foreach MissionState.Rewards(RewardRef, i)
		{
			...
			`log("Reward: " $ RewardState.GetMyTemplateName());
			`log("Reward Name: " $ RewardState.GetRewardString());
			`log("Reward Desc: " $ RewardState.GetRewardDetailsString());
